...
export class HostProfiler {
  ...
  constructor(
    config: HostProfilerConfig = { personality: 'emi', maxWords: 70 }
  ) {
    this.config = config
...
Respond with ONLY the introduction text, nothing else.`
  ...
  private getUserPersonalityContext(
...
Explicitness Level: ${personalityCategory.archetype.explicitness}`
  ...
  async generateHostIntroduction(
    ...
    if (cached) {
      console.log(
        `🎯 Host Profiler - Using cached introduction for ${host.username}`
      )
      return cached
    }

    // Get user's personality category
    const userPersonality = PERSONALITY_CATEGORIES.find(
      (cat) => cat.id === userPersonalityId
    )
    if (!userPersonality) {
    ...
    try {
      console.log(
        `🎯 Host Profiler - Generating introduction for ${host.username}...`
      )
      console.log(
        `🎯 Host Profiler - User personality: ${userPersonality.name}`
      )
...
Create a natural, conversational introduction about this host that explains why they'd be a great match for the user. Focus on their personality (from their bio) and what services they'd naturally be good at.`,
      ...
      console.log(
        `🎯 Host Profiler - Raw AI response for ${host.username}:`,
        response
      )
      ...
      console.log(
        `✅ Host Profiler - Generated and cached introduction for ${host.username}`
      )

      return introduction
    } catch (error) {
      console.error(
        `❌ Host Profiler - Error generating introduction for ${host.username}:`,
        error
      )
      ...
      console.log(
        `🔄 Host Profiler - Using fallback introduction for ${host.username}`
      )
    ...
    for (let i = 0; i < hosts.length; i += batchSize) {
      ...
      const batchPromises = batch.map((host) =>
        this.generateHostIntroduction(host, userPersonalityId)
          ...
          .catch((error) => {
            console.error(
              `❌ Host Profiler - Batch error for ${host.username}:`,
              error
            )
            return null
          })
  ...
  clearCacheForPersonality(userPersonalityId: string): void {
    const keysToDelete = Array.from(this.cache.keys()).filter((key) =>
      key.endsWith(`_${userPersonalityId}`)
    )

    keysToDelete.forEach((key) => this.cache.delete(key))
    console.log(
      `🗑️ Host Profiler - Cleared ${keysToDelete.length} cached introductions for personality ${userPersonalityId}`
    )
  }
  ...
  saveCacheToStorage(): void {
    try {
      ...
      console.log(
        `💾 Host Profiler - Saved ${cacheData.length} cached introductions to localStorage`
      )
    } catch (error) {
      console.error(
        '❌ Host Profiler - Error saving cache to localStorage:',
        error
      )
    }
  }
  ...
  loadCacheFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `📥 Host Profiler - Loaded ${this.cache.size} cached introductions from localStorage`
        )
      }
    } catch (error) {
      console.error(
        '❌ Host Profiler - Error loading cache from localStorage:',
        error
      )
      this.cache = new Map()
    }
